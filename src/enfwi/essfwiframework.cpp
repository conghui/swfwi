...
EssFwiFrameworkOld::EssFwiFrameworkOld(Damp4t10d &method, const std::vector<float> &_wlt,
...
void EssFwiFrameworkOld::epoch(int iter, int ivel) {
...
void EssFwiFrameworkOld::writeVel(sf_file file) const {
//  fmMethod.sfWriteVel(file);
  fmMethod.sfWriteVel(this->fmMethod.getVelocity().dat, file);
}
