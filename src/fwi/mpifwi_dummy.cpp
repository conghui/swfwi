...
/*< scale gradient >*/
{
  ...
  for (ix = 1; ix < nx - 1; ix++) {
    for (iz = 1; iz < nz - 1; iz++) {
      a = vv[ix][iz];
      if (precon) {
        a *= sqrtf(illum[ix][iz] + SF_EPS);  /*precondition with residual wavefield illumination*/
      }
      grad[ix][iz] *= 2.0 / a;
    }
  }
...
/*< gaussian bell smoothing for z-axis >*/
{
  ...
  for (ix = 0; ix < nx; ix++)
    for (iz = 0; iz < nz; iz++) {
      s = 0.0;
      for (i = -rbell; i <= rbell; i++) if (iz + i >= 0 && iz + i < nz) {
...
/*< gaussian bell smoothing for x-axis >*/
{
  ...
  for (ix = 0; ix < nx; ix++)
    for (iz = 0; iz < nz; iz++) {
      s = 0.0;
      for (i = -rbell; i <= rbell; i++) if (ix + i >= 0 && ix + i < nx) {
...
int main(int argc, char *argv[]) {
  ...
  Logger::instance().init("fwi", boost::log::trivial::debug, Logger::NO_TIMESTAMP, rank);
  ...
  for (iter = 0; iter < niter; iter++) {
    if (rank == 0 && verb) {
      start = MPI_Wtime();  /* record starting time */
    }
